...
def main(cfg : DictConfig) -> None:
    ...
    print(motion_file)
    motion_data = joblib.load(motion_file)  #motion_file就是处理后的关节旋转数据，和urdf的自由度的数量是一样的，h1就是19个自由度
    ...
    with mujoco.viewer.launch_passive(mj_model, mj_data, key_callback=key_call_back) as viewer:
        ...
        while viewer.is_running():
            ...
            #mj_data.qpos[:3] = [0,0,0]           
            mj_data.qpos[:3] = curr_motion['root_trans_offset'][curr_time]
            mj_data.qpos[3:7] = curr_motion['root_rot'][curr_time][[3, 0, 1, 2]]
            mj_data.qpos[7:] = curr_motion['dof'][curr_time]                             #这里给root关节添加 x y z位置， w x y z欧拉角，然后给关节赋值
            
            # print("curr_motion['dof'][curr_time]---------------")
            # print(curr_motion['dof'][curr_time])
            # print("-------------------------------------------")    
            
            #既然已经可以获得期望的关节角度了，就可以构建
            ...
            for i in range(joint_gt.shape[1]):
                viewer.user_scn.geoms[i].pos = joint_gt[curr_time, i]   #这部分是从smpl模型中获取关节位置，就是可视化中的红色球体
